...
public class MethodBehaviorJsonAdapter implements JsonSerializer<MethodBehavior>, JsonDeserializer<MethodBehavior> {
  ...
  private static MethodYield yieldsFromJson(MethodBehavior behavior, int arity, JsonObject methodYield) {
    ...
    if (methodYield.get(JSON_IS_EXCEPTIONAL).getAsBoolean()) {
      ...
      happyPathYield.setResult(methodYield.get(JSON_RESULT_INDEX).getAsInt(), constraintsByDomainFromJson(methodYield.get(JSON_RESULT_CONSTRAINTS).getAsJsonArray()));
  ...
  private static List<ConstraintsByDomain> constraintsFromJson(int arity, JsonArray parameterConstraints) {
    ...
    parameterConstraints.forEach(constraint -> parametersConstraintsByDomain.add(constraintsByDomainFromJson(constraint.getAsJsonArray())));
    return parametersConstraintsByDomain;
  }

  private static ConstraintsByDomain constraintsByDomainFromJson(JsonArray jsonConstraintsByDomain) {
    ConstraintsByDomain constraintsByDomain = ConstraintsByDomain.empty();
    for (int i = 0; i < jsonConstraintsByDomain.size(); i++) {
      String domainConstraint = jsonConstraintsByDomain.get(i).getAsString();
